...
public class CartesianCoordinate implements ICoordinate<CartesianCoordinate>,
	...
	static public double maxMoveDistance = .5;
	static public double minMoveDistance = 0.00005;
	static public int nbDimension = Main.dimension;
	static public double defaultMoveDist = 0.3;
	...
	private double landa = 0.01;
	private double gamma = 0.001;

	public CartesianCoordinate() {
		this.position = new double[nbDimension];
		this.forceVect = null;
	...
	public CartesianCoordinate(double[] coord) {
		this();
	...
	public void applyForce() {
		if (this.forceVect != null) {
			...
			if (squareDist != 0) {
				...
				//end of computing the direction.
				if (this.previousForceVect != null) {
					...
					//end of computing the inner product.
					if (scalar > 0) {
	...
	/*public void applyForce(CoordinatesPeer p) {
		if (this.forceVect != null) {
			//computing the direction.
			double squareDist = 0;
			for (int i = 0; i < nbDimension; i++) {
			...
			if (squareDist != 0) {
				...
				if (this.previousForceVect != null) {
					...
					//end of computing the inner product.
					if (scalar > 0) {
						p.stepLength += p.stepLength * scalar
								* faster;
					} else if (scalar < 0) {
						p.stepLength += p.stepLength * scalar
								* slower;
					...
					//this.moveDistance *= 0.9;
					p.stepLength = Math.max(minMoveDistance, Math.min(
							maxMoveDistance, p.stepLength));
				}
				for (int i = 0; i < nbDimension; i++) {
					this.position[i] += p.stepLength * this.forceVect[i];
				}
			}
			double[] temp = this.previousForceVect;
			this.previousForceVect = this.forceVect;
	...
	public double distanceMinkowski(CartesianCoordinate c, double k) {
		double minkowskiDist = 0;
		for (int i = 0; i < nbDimension; i++) {
			double diff = this.position[i] - c.position[i];
			minkowskiDist += Math.pow(diff, k);
		}
		return Math.pow(minkowskiDist,(1/k));
	}
	...
	public void storeBodyForce(CoordinatesPeer c, CoordinatesPeer thisPeer, double value) {
		if (this.forceVect == null) {
			this.forceVect = new double[nbDimension];
		}
		CartesianCoordinate movementVector = (CartesianCoordinate) c.getCoordinate().clone();
		movementVector.minus(this);
		while (movementVector.isOrigin()) {
			if (value >= 0) {
				return;
		...
		movementVector.normalize();
		for (int i = 0; i < nbDimension; i++) {
			this.forceVect[i] += (movementVector.position[i] * value * c.mass * thisPeer.mass);
		}
	}
		
	public void storeBodyForceMinkowski(CoordinatesPeer c, CoordinatesPeer thisPeer, double value, double k){
		if (this.forceVect == null) {
			this.forceVect = new double[nbDimension];
		}
		CartesianCoordinate movementVector = (CartesianCoordinate) c.getCoordinate().clone();
		...
		double forceDirectionNormalizer = Math.pow(this.distanceMinkowski(c.getCoordinate(), k),k-1);
				
		for(int i =0; i< nbDimension; i++){
			movementVector.position[i] = Math.pow(movementVector.position[i], (double)(k-1));
			movementVector.position[i] /= forceDirectionNormalizer;
		}
		for (int i = 0; i < nbDimension; i++) {
			this.forceVect[i] += (movementVector.position[i] * value * c.mass * thisPeer.mass);
		}
	...
	public void storeForceMinkowski(CartesianCoordinate c, double value, double k){
		if (this.forceVect == null) {
			this.forceVect = new double[nbDimension];
		}
		CartesianCoordinate movementVector = (CartesianCoordinate) c.clone();
		...
		double forceDirectionNormalizer = Math.pow(this.distanceMinkowski(c, k),k-1);
	...
	public void normalizeMinkowski(int k){
		double abs = 0;
		for(int i =0;i <nbDimension;i++){
			abs += Math.pow(Math.abs(this.position[i]), (double)k);
		}
		double minkDistance = Math.pow(abs, (double)(1/k));
		if (minkDistance != 0) {
			this.div(minkDistance);
		}
	...
	public void maxDimension(){
		...
		double maxIndex = 0;
		for(int i=0;i<nbDimension;i++){
			if (Math.abs(this.position[i]) > max){
				max = this.position[i];
				maxIndex = i;
			}
		}
	...
	private void rand() {
		for (int i = 0; i < nbDimension; i++) {
			this.position[i] = Math.random() - 0.5;
		}
	}
	...
	public void initialize(Point2D p) {
		this.position[0] = p.getX();
		if (nbDimension > 1) {
			this.position[1] = p.getY();
		}
	}
	...
	public double innerProduct(double[] c){
		double res =0;
	...
	public double[] vectorByScalar( double scalar){
		double[] res = Arrays.copyOf(this.position, this.position.length);
		for(int i=0;i<this.position.length;i++){
			res[i] *= scalar;
		}
		return res;
	...
	public void innerProductMFFunction(CartesianCoordinate c, double error){
		double[] thisVect = this.vectorByScalar(landa);
		double[] cVect = c.vectorByScalar(error);
		for(int i=0; i<thisVect.length;i++){
			this.position[i] += gamma*(cVect[i] - thisVect[i]);
		}
	}
	public void tanMFFunction(CartesianCoordinate c){
		
	}
	public void linearMFFunction(CartesianCoordinate c){
