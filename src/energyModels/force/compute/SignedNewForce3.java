...
public class SignedNewForce3 extends AActionTaskFactory<CoordinatesPeer>{
	...
	public SignedNewForce3(double posEdgeScaling, double negEdgeScaling,
		...
		protected void doAction() {
			...
			for (Entry<CoordinatesPeer, Double> pushNeighbor : this.peer.getRepulseNeighbors().entrySet()) {
				if (!this.peer.equals(pushNeighbor)) {
					...
					d = Math.max(d, 0.0001);
					thisPos.storeBodyForce(pushNeighbor.getKey(), this.peer, (-1.0 / d)*negEdgeScaling*pushNeighbor.getValue());
				}
			}
			//neutral force
